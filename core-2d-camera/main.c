...
void platform_on_event(const sapp_event* event)
{
    if (event->type == SAPP_EVENTTYPE_KEY_DOWN)
    {
        ...
        if (event->key_code == SAPP_KEYCODE_A) global_input_data.a_pressed = 1;
        if (event->key_code == SAPP_KEYCODE_S) global_input_data.s_pressed = 1;
    ...
    if (event->type == SAPP_EVENTTYPE_KEY_UP)
    {
        ...
        if (event->key_code == SAPP_KEYCODE_A) global_input_data.a_pressed = 0;
        if (event->key_code == SAPP_KEYCODE_S) global_input_data.s_pressed = 0;
