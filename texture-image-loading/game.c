...
void on_init(void)
{
    ...
    image = rf_load_image_from_file("../../../rayfork-examples/assets/raylib_logo.png", RF_DEFAULT_ALLOCATOR, RF_DEFAULT_ALLOCATOR, RF_DEFAULT_IO);     // Loaded in CPU memory (RAM)
    texture = rf_load_texture_from_image(image);          // Image converted to texture, GPU memory (VRAM)

    rf_unload_image(image, RF_DEFAULT_ALLOCATOR);   // Once image has been converted to texture and uploaded to VRAM, it can be unloaded from RAM
}
