...
void on_init(void)
{
    ...
    cat = rf_load_image_from_file("../../../rayfork-examples/assets/cat.png", RF_DEFAULT_ALLOCATOR, RF_DEFAULT_ALLOCATOR, RF_DEFAULT_IO);             // Load image in CPU memory (RAM)
    rf_image_crop(cat, (rf_rec){ 100, 10, 280, 380 }, RF_DEFAULT_ALLOCATOR);      // Crop an image piece
    rf_image_flip_horizontal(cat);                              // Flip cropped image horizontally
    ...
    font = rf_load_png_font_from_file("../../../rayfork-examples/assets/custom_jupiter_crash.png", RF_DEFAULT_ALLOCATOR, RF_DEFAULT_ALLOCATOR, RF_DEFAULT_IO);
