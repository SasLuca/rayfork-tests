...
void platform_on_event(const sapp_event* event)
{
    ...
    if (event->type == SAPP_EVENTTYPE_KEY_DOWN && event->key_code == SAPP_KEYCODE_C) global_input_data.c_pressed = 1;
    if (event->type == SAPP_EVENTTYPE_KEY_DOWN && event->key_code == SAPP_KEYCODE_R) global_input_data.r_pressed = 1;
    if (event->type == SAPP_EVENTTYPE_KEY_DOWN && event->key_code == SAPP_KEYCODE_SPACE) global_input_data.space_pressed = 1;
    ...
    if (event->type == SAPP_EVENTTYPE_KEY_UP && event->key_code == SAPP_KEYCODE_C) global_input_data.c_pressed = 0;
    if (event->type == SAPP_EVENTTYPE_KEY_UP && event->key_code == SAPP_KEYCODE_R) global_input_data.r_pressed = 0;
    if (event->type == SAPP_EVENTTYPE_KEY_UP && event->key_code == SAPP_KEYCODE_SPACE) global_input_data.space_pressed = 0;
}
